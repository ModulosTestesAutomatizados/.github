...
validate_release_gates() {
  ...
  local default_branch milestone_json issues_json milestone_number prs_json pr_json
  ...
  EPIC_NUMBER=$(node -e 'const list=JSON.parse(process.argv[1]);const epic=list.find(i=>i.title===process.argv[2]&&i.state==="closed"&&!i.pull_request);if(!epic)process.exit(1);process.stdout.write(String(epic.number))' "$issues_json" "$MILESTONE") || {
    echo "Épica $MILESTONE deve estar encerrada" >&2; return 1;
  }
  ...
  pr_json=$(node -e '
    ...
    const [sha, target, release] = process.argv.slice(2);
    const pr=prs.find(p=>p.merged_at&&p.merge_commit_sha===sha&&p.base?.ref===target
      &&[release,"develop"].includes(p.head?.ref)) ??
  ...
  ' "$prs_json" "$GITHUB_SHA" "$TARGET_BRANCH" "$RELEASE_BRANCH") || {
  ...
  if [[ "$(node -p 'JSON.parse(process.argv[1]).head.ref' "$pr_json")" == versioning/* ]]; then
    ...
    [[ "$body" == *"Adapter: $ADAPTER"* && "$body" == *"Project: ${PROJECT_PATH:-.}"* \
      ...
      echo 'PR de versão diverge do adaptador, projeto ou épica do caller' >&2; return 1;
    }
